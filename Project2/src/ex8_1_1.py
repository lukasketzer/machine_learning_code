...
for train_index, test_index in CV.split(X, y):
    ...
    # )
    mod = lm.Ridge(alpha=opt_lambda)
    mod.fit(X_test, y_test)
    
    Error_train_nofeatures[k] = (
        np.square(y_train - mod.predict(X_train)).sum(axis=0) / y_train.shape[0]
    )
    Error_test_nofeatures[k] = (
        np.square(y_test - mod.predict(X_test)).sum(axis=0) / y_test.shape[0]
    )
